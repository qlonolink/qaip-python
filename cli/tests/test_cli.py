...
from qaip_cli._api._common import filter_fields
from qaip_cli._cli import _build_parser, main
from qaip_cli._errors import CLIError
...
class TestClientCredentials:
    ...
    def test_get_client_uses_cli_args(self) -> None:
        ...
        from qaip_cli._utils import get_client

        with _patch("qaip_cli._utils.qaip.Qaip") as mock_qaip:
...
class TestHeaderOption:
    ...
    def test_parse_headers_invalid_raises(self) -> None:
        from qaip_cli._utils import parse_headers
    ...
    def test_header_reaches_request(self) -> None:
        ...
        with patch("qaip_cli._api.crawls.get_client", return_value=client):
            args.func(args)
...
class TestLocalFileGroupsMissingFile:
    ...
    def test_create_sends_file_as_files_multipart_field(
        ...
        with Qaip(
            ...
            with patch("qaip_cli._api.local_file_groups.get_client", return_value=client):
                args.func(args)
...
class TestSourcesDownloadRaw:
    ...
    def test_existing_output_requires_force(
        ...
        with patch("qaip_cli._api.sources.get_client") as mock_get_client:
            with pytest.raises(CLIError) as exc_info:
                args.func(args)
    ...
    def test_streams_to_stdout(
        ...
        with client:
            monkeypatch.setattr("qaip_cli._api.sources.sys.stdout", fake_stdout)
            with patch("qaip_cli._api.sources.get_client", return_value=client):
                args.func(args)
...
class TestCrawlsDownloadRawArchive:
    ...
    def test_duplicate_source_id_rejected_before_request(self) -> None:
        ...
        with patch("qaip_cli._api.crawls.get_client") as mock_get_client:
            with pytest.raises(CLIError) as exc_info:
                args.func(args)
...
class TestMainTypeErrorHandling:
    def test_main_converts_unexpected_kwarg_typeerror(
        ...
        with _patch("qaip_cli._utils.qaip.Qaip", return_value=mock_client):
